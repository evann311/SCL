...
class BertOutput(nn.Module):
    def __init__(self, bert_config, config, use_lora: bool = False):
        ...
        self.use_lora = use_lora
        if self.use_lora:
            self.dense_lora = LoRALayer(bert_config.intermediate_size, bert_config.hidden_size, rank=8, alpha=16)


    def forward(self, hidden_states, input_tensor):
        if self.use_lora:
            hidden_states = self.dense(hidden_states) + self.dense_lora(hidden_states)
        else:
            hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
...
class BertCrossLayer(nn.Module):
    def __init__(self, bert_config, config, use_lora: bool = False):
        ...
        self.seq_len_dim = 1
        self.attention = BertAttention(bert_config, config, use_lora)
        self.is_decoder = bert_config.is_decoder
        self.add_cross_attention = bert_config.add_cross_attention
        self.crossattention = BertAttention(bert_config, config, use_lora)
        self.intermediate = BertIntermediate(bert_config)
        self.output = BertOutput(bert_config, config, use_lora)
